...
def compare(famList):
    ...
    print("Select the second Orbian")
...
def toPasture(famList):
    ...
    orb1 = selectOrbian(famList)
    print("\tFarewell, dear " + orb1.getName(), end="")
    thinking()
    ind = famList.index(orb1)
    famList.pop(ind)
...
def thanosSnap(famList):
    ...
    thinking()
    numtokill = len(famList) // 2
    for ind in range(numtokill):
        target = randint(0,len(famList)-1)
        famList.pop(target)
