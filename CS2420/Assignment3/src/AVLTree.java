...
public class AVLTree<E extends Comparable<? super E>> {
    ...
    public E deleteMin() {
        ...
        //return root.value;
        ArrayList<AvlNode> nodes = findMin(this.root);
        AvlNode minNode = nodes.get(0);
        AvlNode parMinNode = nodes.get(1);

        //AvlNode thing = delete(minNode.value,this.root,this.root);
        //return thing.value;

        if (minNode == parMinNode){
            AvlNode temp = this.root;
            this.root = temp.right;
        }
        else {
            if (parMinNode.value.compareTo(minNode.value) < 0){
                parMinNode.right = minNode.right;
            }
            else {
                parMinNode.left = minNode.right;
            }
            //balance(minNode);
        }
    ...
    private ArrayList<AvlNode> findMin(AvlNode node){
        AvlNode tempNode = node;
        AvlNode tempParent = node;
    ...
    public void printTree(String label) {
        ...
        System.out.println(printTree(this.root,-1));
    }

    private String printTree(AvlNode node, int progress){
        String msg = "";
        if (node != null) {
            progress++;
            printTree(node.right,progress);
            //(node.parent != null)
            String tempMsg = node.value + "(" + heightTree(node) + ")";
            //String tempMsg = node.value + "(" + progress + ")";
            System.out.print(tempMsg.indent(2*progress));

            printTree(node.left,progress);
        }
        return msg;
    }

    private int heightTree(AVLTree<E> tree){
    ...
    private int heightTree(AvlNode node){
        if (node.right == null){
            if (node.left == null){
                ...
                return heightTree(node.left) + 1;
            }
        }
        else if (node.left == null) {
            return heightTree(node.right) + 1;
        }
        else{
            return Math.max(heightTree(node.right), heightTree(node.left)) +1;
        }
    }
