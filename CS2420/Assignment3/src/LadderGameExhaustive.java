...
public class LadderGameExhaustive extends LadderGame {
    ...
    public LadderGameExhaustive(String dictionaryFile) { readDictionary(dictionaryFile);}

    public void play(String start, String end) {
        // TODO: Write some good stuff here
        //String msg = start + " " + "-> " + end + " : ";
        String msg = "Seeking exhaustive solution from " + start + " -> " + end + "\n";
        ...
        Queue<WordInfoExhaustive> parQ = new Queue<>();
        WordInfoExhaustive Start = new WordInfoExhaustive(start,0);
        parQ.enqueue(Start);

        boolean complete = false;
        int EQ = 0;
        while(!parQ.isEmpty() && !complete){
            WordInfoExhaustive shortest = parQ.dequeue();
            ...
            for(String wrd: OneAway){
                if(Objects.equals(wrd, end)){
                    ...
                    WordInfoExhaustive doneLad = new WordInfoExhaustive(wrd, Moves, shortest.getHistory()+ " " + wrd);
                    ...
                    WordInfoExhaustive newLad = new WordInfoExhaustive(wrd,0, shortest.getHistory()+ " " + wrd);
                    parQ.enqueue(newLad);
                    EQ += 1;
                }
            }
